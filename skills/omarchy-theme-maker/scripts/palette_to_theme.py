#!/usr/bin/env python3
"""palette_to_theme.py -- build an Omarchy theme palette from one image.

FIRST PASS ONLY: this script extracts dominant colors via median-cut
quantization (Pillow), decides dark/light mode from weighted luminance, writes
a contrast-checked colors.toml with the canonical Omarchy palette keys, and
installs the image as the theme background. Review the result and hand-tune it
to the artwork's actual visual identity -- quantization picks dominant pixels,
not identity colors, and can pick a wrong accent for brand-driven artwork.

Usage:
  palette_to_theme.py IMAGE [--name NAME] [--mode dark|light|auto]
                          [--out DIR] [--force] [--print]

Output theme dir (default: ~/.config/omarchy/themes/, or DIR if --out):
...
DEFAULT_THEMES_DIR = Path.home() / ".config" / "omarchy" / "themes"
...
def weighted_luminance(hex_colors):
    """Mean lightness weighted by saturation -- a proxy for perceived theme mode."""
    total = 0.0
    for h in hex_colors:
        rh, gh, bh = hex_to_rgb(h)
        _, li, s = colorsys.rgb_to_hls(rh / 255, gh / 255, bh / 255)
        total += li * (0.5 + s)
    return total / len(hex_colors)
...
def main():
    ap = argparse.ArgumentParser(
        description="Build a first-pass Omarchy theme palette (colors.toml + background) from one image.",
        epilog="Output is a starting point: review and hand-tune to the artwork's identity, "
               "then verify with contrast_report.py.",
    )
    ap.add_argument("image", help="Source image (jpg/png/webp/...)")
    ap.add_argument("--name", help="Theme name (default: image filename)")
    ap.add_argument("--mode", choices=["dark", "light", "auto"], default="auto",
                    help="Theme mode (default: auto, from weighted luminance)")
    ap.add_argument("--out", type=Path, default=None, metavar="DIR",
                    help="Write the theme to DIR/<slug>/ instead of the default "
                         f"{DEFAULT_THEMES_DIR}/ (e.g. --out themes for a collection repo checkout)")
    ...
    slug = slugify(name)
    themes_dir = Path(args.out).expanduser().resolve() if args.out else DEFAULT_THEMES_DIR
    ...
    shutil.copy2(image, bg_dir / image.name)
    ...
    print(f"\nNext: hand-tune colors.toml to the artwork's identity, verify with")
    print(f"contrast_report.py, then apply: omarchy theme set {slug}")
